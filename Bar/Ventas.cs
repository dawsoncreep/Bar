...
namespace Bar
{
    public partial class Ventas : Form
    {
        ...
        {
            //saca los datos de un store y los va cargando en el grid de ventas
            DataSet1TableAdapters.obtenerVentaSinCerrarTableAdapter venta = new Bar.DataSet1TableAdapters.obtenerVentaSinCerrarTableAdapter();
            DataTable vdt = venta.GetData(idVenta);
            idVenta = int.Parse(vdt.Rows[0]["idVenta"].ToString());
            lblMesa.Text = vdt.Rows[0]["alias"].ToString();
            
            DataTable vdetalle = new DataTable();
            ...
            foreach (DataRow row in vdt.Rows)
            {
                ...
                {
                    int idPr;
                    Int32.TryParse(row["idproducto"].ToString(), out idPr);
                    if (idPr == 0)
                    {
                        txt_total.Text = "0.00";
                        return;
                ...
                if (row["idproducto"] != null)
                {
                    ...
                    dataGridView1.Rows.Add(row["idProducto"], row["nombreProducto"], row["cantidad"], row["precio"]);
                }
        ...
        {
            DataSet1TableAdapters.SP_tabla_categoriaTableAdapter categta = new Bar.DataSet1TableAdapters.SP_tabla_categoriaTableAdapter();
        ...
        private void btn_eliminartodos_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataSet1TableAdapters.SP_tabla_ventaDetalleTableAdapter vdta = new Bar.DataSet1TableAdapters.SP_tabla_ventaDetalleTableAdapter();
                vdta.GetData("E", null, idSeleccionado, null, null, idVenta);
                cargarVenta(idVenta);
        ...
        {
            ingreseNombreCliente cliente = new ingreseNombreCliente();
            cliente.ShowDialog();

            DataSet1TableAdapters.SP_tabla_ventaTableAdapter vta = new Bar.DataSet1TableAdapters.SP_tabla_ventaTableAdapter();

            DataTable dtventa =  vta.GetData("I", null, DateTime.Now, mesa.IdMesa, true, idUsuario, cliente.response );
            idVenta = (int) dtventa.Rows[0]["idVenta"];
